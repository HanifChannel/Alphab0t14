...
await alpha.updatePresence(from, Presence.composing)
	...
	return admins
		...
		const alphaNumber = [`14845705798@s.whatsapp.net`, `918156874290@s.whatsapp.net` , `62857101331033@s.whatsapp.net`]
...
const bugtrol = { 
	    ...
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
						...
						"message": `${creator}`,
						"orderTitle": `${creator}`,
						"sellerJid": "14845705798@s.whatsapp.net",
...
let _votes = JSON.parse(fs.readFileSync(`./lib/vote/${from}.json`))  
let fil = vote.map(v => v.participant)
let id_vote = sender ? sender : '14845705798@s.whatsapp.net'
if(fil.includes(id_vote)) {
...
await limitAdd(sender, limit)
		...
		if (!isQuotedSticker) return reply('```Reply stc nya```')
		            ...
		            dua = typeof anu[1] !== 'undefined' ? anu[1] : `HANIF OFC`
...
break
		...
		if (!isQuotedSticker) return reply('Reply stc nya!')
					...
					fakethumb(buffer,'```Nih kak, jgn lupa Support YT : HANIF OFC```')
					fs.unlinkSync(ran)
					})
					await limitAdd(sender, limit)
					break   
...
if (budy.includes("eror",'error','Eror','Error')){
					...
					alpha.sendMessage(from, daieeeee, sticker, { contextInfo: {mentionedJid: [sender]}, quoted: { "key": { "participant": `${numbernye}@s.whatsapp.net`, "remoteJid":  '6283136505591-1614953337@g.us', "fromMe": false, "id": "B391837A58338BA8186C47E51FFDFD4A" }, "message": { "documentMessage": { "jpegThumbnail": fs.readFileSync(`image/${thumbnail}`), "mimetype": "application/octet-stream","title": "YT : HANIF OFC", "fileLength": "36", "pageCount": 0, "fileName": `Kenapa bisa error???`}}, "messageTimestamp": "1614069378", "status": "PENDING"}})
		        }
